...
| Authentication | Partial | Registration, email-verification/reset challenges, Argon2id passwords, TOTP MFA, server-side refresh-token rotation/replay revocation, rate limits and progressive lockouts are implemented. Capability-driven account actions, verification resend and shared desktop/browser account flows are implemented. Deployment defaults keep public registration disabled until SMTP delivery, operational monitoring, and PostgreSQL end-to-end evidence are approved |
