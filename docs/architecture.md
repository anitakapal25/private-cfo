...
- **Milestone 2 verified financial memory — Implemented:** Authenticated users record calendar-month income, expenses and debt payments or dated snapshots for other facts. Candidates never enter calculation context before explicit confirmation; batch decisions are transactional, replacements preserve history, and purpose-scoped calculations refuse to combine monthly values from different periods.
