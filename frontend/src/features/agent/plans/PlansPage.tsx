...
const PlansPage: React.FC<Props> = ({ token, conversationId, onConversationCreated, onAsk }) => {
  ...
  return <section className="plans-page" aria-labelledby="plans-title">
    <header className="plans-heading"><div><p className="eyebrow">MY PLAN</p><h2 className="heading-with-icon page-title-with-icon" id="plans-title"><Target aria-hidden="true"/>Turn intentions into manageable actions</h2><p>Track only the actions you choose and confirm. Progress is based on your check-ins.</p></div><span><Check/> Only confirmed actions are added</span></header>
    ...
    <div className="plans-layout"><div className="plan-actions"><div className="plan-section-title"><h3 className="heading-with-icon"><CalendarDays aria-hidden="true"/>Active actions</h3>{!creating && <Button type="button" size="sm" onClick={() => setCreating(true)}><Plus/> Create action</Button>}</div>
